...
cd .base_repo
git diff  $baseCommitHash..HEAD -- . ':(exclude).cache' > sync_patch.diff
...
cd ..
if ! git apply --reject --whitespace=fix .base_repo/sync_patch.diff; then
    rm .base_commit_latest  # remove the latest base commit.
    echo "Failed to apply patches. Please check the conflict files and resolve them manually!"
    echo "无法应用补丁，请检查冲突文件并手动解决！"
    exit 0
fi

# Step 6: remove the `.base_repo` folder and apply the latest base_commit.
rm -rf .base_repo
